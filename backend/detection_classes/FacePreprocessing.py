...
class FacePreprocessing:
    def __init__(self):
        ...
        self.max_threshold_distance = 30
    ...
    def gaze(self,face, result_facePoint):
        try:
            ...
            face_3d = []
            face_2d = []
            if not hasattr(self, 'gaze_result'):
                self.gaze_result = {
                    "Left": 0,
                    "Right": 0,
                    "Up": 0,
            ...
            if not hasattr(self, "prev_gaze"):
                self.prev_gaze="Forward"
            if not hasattr(self, "gaze_locked"):
                self.gaze_locked = False  # Prevent duplicate counting
            if result_facePoint.multi_face_landmarks:
                for face_landmarks in result_facePoint.multi_face_landmarks:
                    ...
                    if self.y < -10:
                        ...
                        cv.putText(face, text, (top_left_x, top_left_y + int(height * 0.18)), cv.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 255), thickness)
                        curr_gaze="Left"
                    elif self.y > 10:
                    ...
                    if curr_gaze != self.prev_gaze:
                        if curr_gaze == "Forward":
                            self.gaze_locked = False  # Unlock when looking forward
                        elif not self.gaze_locked:
                            self.gaze_result[curr_gaze] += 1
                            self.gaze_locked = True  # Lock counting to avoid duplicate frames
                    self.prev_gaze = curr_gaze
                    toast=text
                    # Update cheat flags
                    self.X_AXIS_CHEAT = 1 if self.y < -10 or self.y > 10 else 0
                    self.Y_AXIS_CHEAT = 1 if self.x < -5 else 0
                    nose_3d_projection, _ = cv.projectPoints(
                        nose_3d, rot_vec, trans_vec, cam_matrix, dist_matrix
                    )
                    p1 = (int(nose_2d[0]), int(nose_2d[1]))
                    p2 = (
                        int(nose_3d_projection[0][0][0]),
                ...
                toast=text
                cv.putText(face, text,(top_left_x, top_left_y + int(height * 0.26)), cv.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 255), thickness)
                if self.prev_gaze != "Obstruct":
                    self.gaze_result["Obstruct"] += 1
